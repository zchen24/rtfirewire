 ...
 * @ingroup nodemgr
 * @file
 * Implementation of nodemgr module
  */
/**
 * @defgroup nodemgr
 * node manager module
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_scan_root_directory
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_scan_unit_directory
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_process_unit_directory
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_process_root_directory
 * for now, we only parse the vendor id, capability, and unit directory.
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_claim_unit_directory
 * to claim a new unit directory to the dirver. 
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_release_unit_directory
* remove the cached unit directory structure.
 */
...
/**
 * @ingroup nodemgr
 * @anchor hpsb_release_unit_directory
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_free_unit_directories
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_match_driver
 * @return the device id if the protocol driver matches 
 * the unit directory in one of the 4 matching ways. 
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_bind_drivers
 * to bind a local driver to the unit directory on remote node
 */
...
/**
 * @ingroup nodemgr
 * @anchor hpsb_unregister_protocol
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_process_config_rom
 */
static void nodemgr_process_config_rom(struct node_entry *ne, 
				       quadlet_t busoptions)
{
...
/**
 * @ingroup nodemgr
 * @anchor read_businfo_block
 * @param buffer_length the length of buffer in quadlet
*/
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_remove_node
 * called when a node is removed, including 
 * removing the unit directory and the node_entry struct. 
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_node_probe_cleanup
 * cleanup the probed
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_node_probe
 * to probe all the nodes attached to a certain host
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_do_irm_duties
 * Because we are a 1394a-2000 compliant IRM, we need to inform all the other
 * nodes of the broadcast channel.  (Really we're only setting the validity
 * bit). Other IRM responsibilities go in here as well.
*/
static int nodemgr_do_irm_duties(struct hpsb_host *host)
{
	quadlet_t bc;
...
/**
* @ingroup nodemgr 
 * @anchor nodemgr_check_irm_capability
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_host_thread
 * the kernel thread keeping its related host's info uptodate.
 */
...
static void nodemgr_add_host(struct hpsb_host *host)
{
	...
	init_completion(&hi->exited);
        sema_init(&hi->reset_sem, 1);

	sprintf(hi->daemon_name, "knodemgrd_%d", host->ifindex);
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_host_reset
 * called when a host's bus is resetted
* it just up the reset sem, soto wakeup the kernel thread. 
 */
...
/**
 * @ingroup nodemgr
 * @anchor nodemgr_remove_host
 * called when a host is removed
 */
...
#define PROC_ENTRY "nodemgr"

int init_ieee1394_nodemgr(void)
{
	if(!create_proc_read_entry(PROC_ENTRY, 0444, rtfw_procfs_entry, raw1394_read_proc, NULL))
		HPSB_ERR("Can't create nodemgr procfs entry\n");
...
void cleanup_ieee1394_nodemgr(void)
{
	...
	remove_proc_entry(PROC_ENTRY, rtfw_procfs_entry);
}

module_init(init_ieee1394_nodemgr);
module_exit(cleanup_ieee1394_nodemgr);
