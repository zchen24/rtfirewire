 ...
 * @ingroup csr
 * @internal
 * basically, csr is no different then a highlevel driver,
 * so it needs a hpsb_highlevel structure
 */
 ...
 * @ingroup csr
 * @anchor map_ops
 * ops for topology map and speed map
 */
 ...
 * @ingroup csr
 * @anchor host_reset
 * Called when a reset occurs in host
 *
 * @note see @ref bus reset section of "Overview of Real-Time Firewire Stack". 
 */
static void host_reset(struct hpsb_host *host)
{
 ...
 * @ingroup csr
 * @anchor add_host
 * Called when a new host is added to csr highlevel module
 *
 * This function mapps several related address space: 
 *  - base register
 *  - config rom
 *  - fcp command and response
 *  - topology map
 *  - speed map
 * Also it initialize the address spaces:
 *  - initialize the base register
 *  - initialize the config rom (bus name, node capability, bus info block, and root directory, etc)
 * 
 * @note for more info, see @ref csr section of "Overview of Real-Time Firewire Stack". 
 */
static void add_host(struct hpsb_host *host)
{
 ...
 * @ingroup csr
 * @anchor remove_host
 * Called when a host is removed from csr module
 *
 * @note for more info, see @ref csr section of "Overview of Real-Time Firewire Stack". 
 */
static void remove_host(struct hpsb_host *host)
...
/**
 * @ingroup csr
 * @anchor hpsb_update_config_rom
 * Update the configrom of a certain host
 * 
 ...
 * @ingroup csr
 * @anchor read_regs
 * Called when access to base registers occurs
 *
 * @param buffer - the buffer to store read info
 * @param addr - the 48-bit address
 * @param length - the length of required data
 * @param fl - not used. 
 *
 * @return RCODE_COMPLETE on success, other values:
 *   - RCODE_ADDRESS_ERROR
 *   - RCODE_TYPE_ERROR
 * @note see more info in @ref csr section of "Overview of Real-Time Firewire Stack". 
 */
 ...
 * @ingroup csr
 * @anchor lock_regs
 * Called when lock access to base registers occurs
 * if device driver implements "hw_csr_reg" routine,
 * the lock operation will be done in hardware. 
 *
 * @param store - the buffer to store read info
 * @param addr - the 48-bit address
 * @param data arg - the new and old data. 
 * @param extcode - related lock operation code. 
 * @param fl - not used. 
 *
 * @return RCODE_COMPLETE on success, other values:
 *   - RCODE_ADDRESS_ERROR
 ...
 * @ingroup csr
 * @anchor lock64_regs
 * Called when lock64 access to base registers occurs
 * if device driver implements "hw_csr_reg" routine,
 * the lock operation will be done in hardware. 
 *
 * @param store - the buffer to store read info
 ...
 * @ingroup csr
 * @anchor write_fcp
 * Called when fcp "registers" are accessed
 * 
 * @param length - packet length, no larger than 512. 
 * 
...
/**
 * @ingroup csr
 * @anchor read_config_rom
 * Called when config rom is accessed
 * 
...
/**
 * @ingroup csr highlevel routines
 * @anchor allocate_addr_range
 */
static u64 allocate_addr_range(u64 size, u32 alignment, void *__host)
{
